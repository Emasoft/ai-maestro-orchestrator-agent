...
Examples:
  ...
  %(prog)s --patch --dry-run    # run every validation step fully, then
    ...
    staged: list[str] = []
    # uv.lock is regenerated by _sync_uv_lock() in Step 9 so it tracks the
    # new version. Without staging it here, the release commit ships a
    # stale lockfile and the next run hits the Step 1 dirty-tree branch
    # and cuts an extra `chore: update uv.lock` housekeeping commit.
    for name in (
