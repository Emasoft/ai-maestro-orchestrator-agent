...
>
> **3-pillars contract (proactive):** You hold **no direct pillar-write authority** — never edit `design/tasks/*.md`, `design/proposals/*.md`, or the PRRD, and never invoke `ama-trdd-*` / `ama-prrd-*` / `ama-kanban-render` yourself. Recall prior task/coordination context via the memory contract above using pillar vocabulary (TRDD id, module name, blocker symptom) before acting. When your coordination surfaces an integration decision that deviates from a baseline, crosses a team/project boundary, or touches release/production, self-classify it at the appropriate approval floor (`~/.claude/rules/trdd-approval-tiers.md`) and flag it explicitly rather than approving unilaterally. Return any task-state finding, proposed TRDD content, or approval-floor judgment to the main agent, which owns all pillar writes and the Tier 0→COS→MANAGER→USER approval ladder.
