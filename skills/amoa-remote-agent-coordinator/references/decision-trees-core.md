...
Use this tree every time an agent reports a failure, error, or blocker during task execution. The tree determines whether AMOA should ask the agent to retry the task, try a different approach, or escalate the problem to a higher-level agent (AMCOS or AMAA).

### 1.2 Input variables AMOA must gather

| Variable | Type | How to obtain |
|----------|------|---------------|
| `retry_count` | Integer (0+) | Count of how many times this specific agent has already retried this specific task. AMOA tracks this in the task record. |
...
Agent reports an issue with the task
    ...
    │     ├─ Yes → ESCALATE to AMCOS immediately
    ...
    │               │         ├─ Yes (same error each time) → ESCALATE to AMCOS
    ...
    │                         │         Action: escalate to AMCOS with message:
...
|------|--------|------------------|
| ESCALATE to AMCOS immediately | Send urgent message to AMCOS via AI Maestro with priority `urgent`. Mark task status as `BLOCKED`. | Use the escalation template from [escalation-procedures.md](./escalation-procedures.md) |
| ESCALATE (systematic issue) | Send high-priority message to AMCOS with all error logs attached. Mark task as `BLOCKED`. | Include the phrase: "Systematic failure detected after [N] retries with identical error." |
...
| ESCALATE with time warning | Send high-priority message to AMCOS requesting guidance. Keep task status as `IN_PROGRESS` but flag it. | Include: time elapsed, time remaining, retry count, last error summary. |
...
AMOA sends: "This is retry 3 of 3. Previous error: TypeError on property 'x' of undefined. The input SVG element may not have x/y attributes. Please add null checks for element.getAttribute('x') before accessing the value. Remaining time budget: 95 minutes."
...
Use this tree when an agent has gone silent -- it has not responded to messages or provided progress updates within the expected polling interval. This tree determines whether AMOA should wait longer, send a reminder, or reassign the task to a different agent.

### 2.2 Input variables AMOA must gather

| Variable | Type | How to obtain |
|----------|------|---------------|
| `no_response_minutes` | Integer | Minutes since the agent's last message or progress update. AMOA tracks this from AI Maestro message timestamps. |
...
|------|--------|-----------|
| REASSIGN immediately | Create new task assignment with all original context plus any partial results from unresponsive agent. Use [op-prepare-task-delegation.md](./op-prepare-task-delegation.md). | Notify AMCOS about the unresponsive agent for potential health check. |
| ESCALATE to AMCOS (no agents) | Send urgent message to AMCOS requesting a new agent be spawned or an existing one freed up. | Block on AMCOS response before proceeding. |
...
AMOA sends: "No response received for 35 minutes on task TSK-042 (write unit tests for bounding box module). Please reply with status within 15 minutes or the task will be reassigned."
...
| Variable | Type | How to obtain |
|----------|------|---------------|
| `response_A` | Text summary | The first agent's result or recommendation, summarized by AMOA. |
| `response_B` | Text summary | The second agent's result or recommendation, summarized by AMOA. |
| `conflict_type` | One of: `implementation`, `design`, `test_result` | AMOA categorizes the conflict. `implementation` means two different code approaches. `design` means disagreement on architecture or patterns. `test_result` means one agent says tests pass and the other says they fail. |
...
Two or more agents have provided contradictory results
    ...
    │               │               └─ Results still conflict → ESCALATE to AMIA
    │               │                    Reason: integration/environment issue
    │               │                    Include: both verbose test logs
    │               │
    │               └─ No (conflict_type is "design") → ESCALATE to AMAA
    │                     Reason: design decisions are AMAA's responsibility
    │                     Action: send high-priority message to AMAA with:
    │                     - Both agents' positions summarized
    │                     - The specific design question to resolve
    │                     - Any constraints from existing architecture
    │                     Block task until AMAA responds
```
...
| ESCALATE to AMAA (implementation) | Send detailed comparison to AMAA. Pause both agents' work on the conflicting area. | AMAA, both agents (told to pause), AMCOS (informed of delay). |
| REQUEST RE-RUN | Send identical re-run instructions to both agents. Wait for both results before deciding. | Both agents. |
| ESCALATE to AMIA (persistent test conflict) | After re-run still conflicts, send to AMIA for environment/integration investigation. | AMIA, both agents, AMCOS. |
| ESCALATE to AMAA (design) | Send design question to AMAA immediately. Do not attempt to resolve design conflicts yourself. | AMAA, both agents (told to pause). |
...
AMOA evaluates: The regex approach is 45 lines, no dependencies, but fragile with edge cases. The DOM approach is 30 lines, uses an existing dependency (xmldom), and handles edge cases naturally. AMOA chooses the DOM approach (fewer lines, handles edge cases, uses existing dependency).

AMOA notifies both: "Both parsing approaches work correctly. Proceeding with the DOM-based approach from programmer-002 because it handles edge cases more robustly and reuses an existing project dependency. Programmer-001, your regex approach was valid -- please archive it as an alternative in the design notes."
...
Use this tree at the end of each verification loop (loops 1 through 4, and the optional 5th check) in the 4-loop verification protocol. Each time a verification pass completes, AMOA must decide what to do with the results.
...
Scenario: Loop 3 of 4 has just completed for a CSS rendering module. The verifier (an AMIA agent) found 2 issues: one minor (inconsistent variable naming) and one major (the module fails on SVG elements with percentage-based widths). The agent provided thorough evidence including test files and error output.
...
Use this tree when an agent that was previously marked as unresponsive or failed comes back online (recovers) while a replacement agent is already working on its task. AMOA must decide whether to keep the replacement or revert to the original agent.
...
| `task_overlap_risk` | One of: `none`, `low`, `high` | Risk that switching agents mid-task will cause conflicts (e.g., both wrote to the same files, both committed to the same branch). AMOA assesses this from both agents' work logs. |
...
Original agent has recovered while replacement is working
    ...
    │     │    4. Log the incident for AMCOS review
    │     │
...
Use this tree every time a new request or task arrives. The tree determines whether AMOA should handle it directly (because it is simple enough) or delegate it to a specialized agent (because it requires coding, research, or multi-step work).
...
| `task_complexity` | One of: `simple`, `medium`, `complex` | Simple: can be answered in 1-2 messages with information AMOA already has. Medium: requires some investigation but no specialized skills. Complex: requires deep analysis, multiple steps, or specialized tools. |
| `amoa_current_load` | One of: `light` (0-2 active tasks), `moderate` (3-5), `heavy` (6+) | Count of tasks AMOA is currently actively coordinating. |
...
New request or task has arrived
    ...
    │               │
    │               ├─ Yes → HANDLE DIRECTLY
    │               │         AMOA responds immediately from its own knowledge
    ...
    │                         │    │     ├─ Yes → Delegate to AMAA (Architect)
    │                         │    │     └─ No ↓
    │                         │    │
    │                         │    ├─ Is it a quality/review question?
    │                         │    │     ├─ Yes → Delegate to AMIA (Integrator)
    │                         │    │     └─ No ↓
    │                         │    │
    ...
    │                              "Send a progress report to AMCOS")
    │                              AMOA handles these operational tasks itself
```
...
Scenario: AMCOS sends AMOA a message: "The SVG bounding box library needs a new function to calculate the combined bounding box of multiple overlapping elements. Please get this implemented."
...
Use this tree during the post-task interview phase, after an agent submits a completed task and AMOA reviews the deliverable. If AMOA finds issues and issues a REVISE verdict (asking the agent to fix problems and resubmit), this tree determines how many REVISE cycles are acceptable before escalating.

### 7.2 Input variables AMOA must gather

| Variable | Type | How to obtain |
|----------|------|---------------|
| `revise_count` | Integer (0+) | Number of times AMOA has issued REVISE for this specific task to this specific agent. Tracked in the task record. |
...
| `agent_response_quality` | One of: `improving`, `stagnant`, `declining` | Whether the agent's revisions are getting better, staying the same, or getting worse with each REVISE. AMOA compares the issues found across REVISE iterations. |
...
```
AMOA has issued a REVISE verdict on an agent's deliverable
    │
    ├─ Is revise_count >= 3?
    │     │
    ...
    │     │     │     │
    │     │     │     ├─ Yes → ESCALATE TO AMCOS (agent capability issue)
    ...
    │     │     │     │    misunderstanding.
    │     │     │     │    Action:
    │     │     │     │    1. Send AMCOS: "Agent [name] has failed to resolve
    ...
    │     │     │     │    3. Pause the task until AMCOS responds
    │     │     │     │
    │     │     │     └─ No (different issues each time) → ESCALATE TO AMAA
    │     │     │          Reason: the task instructions or design may be unclear,
    │     │     │          causing the agent to fix one thing and break another
    │     │     │          Action:
    │     │     │          1. Send AMAA: "Task [ID] has gone through [N] REVISE
    ...
    │     │     │          3. Pause until AMAA provides updated spec
    │     │     │
    │     │     └─ (not reached -- all branches covered)
    │     │
    │     └─ No (revise_count < 3)
    ...
    │           │     │
    │           │     ├─ Yes → ESCALATE TO AMAA immediately
    ...
    │           │     │    Action:
    │           │     │    1. Send AMAA the specific design question
    │           │     │    2. Tell the implementing agent to pause
    │           │     │    3. Once AMAA responds, send updated guidance to agent
...
|------|--------|----------------------|
| ESCALATE TO AMCOS (capability) | Send detailed failure report to AMCOS. Pause task. Wait for AMCOS to either reassign, provide training, or adjust the task. | AMCOS should respond within 30 minutes for P0/P1, 2 hours for P2/P3. |
| ESCALATE TO AMAA (unclear spec) | Send spec ambiguity report to AMAA with all evidence. Pause task. Wait for updated spec. | AMAA should respond within 1 hour. |
| ESCALATE TO AMAA (design issue) | Send design question immediately, even on first REVISE. Do not count as a REVISE cycle. | AMAA should respond within 1 hour. |
| SEND REVISE WITH GUIDANCE | Send detailed fix instructions. Increment counter. Wait for resubmission. | Agent should resubmit within the deadline set by AMOA (typically 1-2 hours). |

### 7.5 Concrete example

Scenario: Agent `svgbbox-programmer-001` submitted a completed SVG text measurement function. AMOA's review found that the function does not account for font-size inheritance from parent elements. This is the second REVISE (revise_count = 2). The first REVISE was about a different issue (missing viewBox handling) which was fixed. The current issue type is `implementation`. The agent's response quality is `improving` (the first issue was fixed correctly).
