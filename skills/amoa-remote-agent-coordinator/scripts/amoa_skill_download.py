...
Usage:
    python amoa_skill_download.py download --url URL --task-id TASK_ID --category CATEGORY
    python amoa_skill_download.py init --project-root PATH
    python amoa_skill_download.py lookup --task-id TASK_ID
    python amoa_skill_download.py verify --project-root PATH
"""
...
def init_storage(project_root: Path) -> None:
    """Initialize the AMOA design storage directory structure."""
    storage_root = get_storage_root(project_root)

    print(f"Initializing AMOA design storage at: {storage_root}")
    ...
    gitkeep.write_text("# AMOA design document storage - do not delete this folder\n")

    # Update .gitignore if in git repo
    gitignore_path = project_root / ".gitignore"
    gitignore_entry = "\n# AMOA Design Document Storage (local cache)\ndesign/\n!design/.gitkeep\n"
    ...
    print("AMOA design storage initialized successfully")
...
def main() -> int:
    ...
    if args.command == "init":
        ...
        if args.json:
            ...
            print("\n=== AMOA Design Storage Verification Report ===\n")
