...
|-------|-------|----------|
| No available agents | All agents at capacity or offline | Wait for agent capacity or escalate to the MAESTRO |
| Circular dependency detected | Task A blocks B, B blocks A | Report to the MAESTRO for manual resolution |
| Agent does not ACK assignment | Agent unresponsive or hibernated | Send reminder, then escalate to **amoa-progress-monitoring** |
| Skill mismatch | No agent has required toolchain | Escalate to the MAESTRO or reassign with training |
